...
const STATUS_STEPS = [
  { key: "servicing", label: "Servicing", icon: "🔧" },
  { key: "serviced", label: "Serviced", icon: "✨" },
  { key: "ready", label: "Ready to Deliver", icon: "✅" },
  { key: "delivered", label: "Delivered", icon: "🚚" },
];

function getStepIndex(vehicle) {
  if (vehicle.status === "Dispatched") return 3;
  if (vehicle.status === "Serviced") {
    if (vehicle.paymentStatus === "Paid") return 2;
    return 1;
  }
  return 0;
}
            ...
            <div className="active-section">
              ...
              {activeVehicles.length === 0 ? (
                ...
                activeVehicles.map((vehicle) => {
                  const stepIndex = getStepIndex(vehicle);
                  return (
